<template lang="">
  ...
  <div class="container mt-3">
    ...
    <select class="form-control form-control-sm" v-model="userInfo.role_id">
      ...
      <option v-for="(role, index) in roleList" :value="role.role_id" :key="role.role_id" :selected= "role.id == userInfo.role_id">{{ role.role_name }}</option>
    </select>
...
export default {
  ...
  created() {
    ...
    me.axios.get(`/api/users/${link_params.id}`)
    .then(res => {
      // console.log(res.data)
  ...
  methods: {
    saveUser() {
      ...
      me.axios.put(`/api/users/${link_params.id}`, me.userInfo)
      .then(res => {
        console.log(res.data)
      }).catch(err => {
        // console.log(err)
